reagent-name-blood =  Sangue.

reagent-desc-blood =  Espero que seja ketchup.

reagent-name-insect-blood =  Sangue de inseto

reagent-desc-insect-blood =  Isso é nojento, parece quase vivo?

reagent-name-slime = slime

reagent-desc-slime =  Você pensou que era sangue gradiente no início, mas estava enganado.

reagent-name-sap = Seiva

reagent-desc-sap =  Sangue de árvore pegajoso e doce.

reagent-name-hemocyanin-blood =  Sangue azul

reagent-desc-hemocyanin-blood =  Contém cobre em vez de ferro que dá uma cor azul distinta.

reagent-name-ammonia-blood =  Sangue anaeróbio

reagent-desc-ammonia-blood =  Nada mais em toda a galáxia cheira tão terrível.

reagent-name-sulfur-blood =  Sangue sulfúrico.

reagent-desc-sulfur-blood =  Parece quase ácido.

reagent-name-zombie-blood =  Sangue de zumbi.

reagent-desc-zombie-blood =  Não aconselharia comer, pode ser usado para criar uma inoculação contra a infecção.

reagent-name-unholy-blood =  Sangue profano

reagent-desc-unholy-blood = Uma substância amaldiçoada que pulsa com energia sombria. Aqueles tocados pelo culto sangram este fluido profano.

reagent-name-ichor = Ichor

reagent-desc-ichor =  Uma química regenerativa extremamente potente, aperfeiçoada pela evolução da fauna espacial, produzida no sistema digestivo do dragão, é vista como uma mercadoria exótica devido ao esforço gigantesco de caça.

reagent-name-fat = Gordura

reagent-desc-fat =  Não importa como foi obtido, sua aplicação é importante.

reagent-name-vomit = Vômito

reagent-desc-vomit =  Dá para ver alguns pedaços da última refeição de alguém nela.

reagent-name-grey-matter =  Matéria cinzenta

reagent-desc-grey-matter = Suco de pensamento, o que vaza dos seus ouvidos.
